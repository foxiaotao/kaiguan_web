...
function init(){
	...
	setTimeout(function(){
		if(token == null || token == "" || token == undefined || token == "null"){
			...
			loadUserService(postdata);//type=net
		}
	}, 500)
            ...
            if(flag=="0"){
            	...
            	if(serverLangList.length > 0){
	            	for(var i=0; i < serverLangList.length ; i++){
	            		var serverLang = serverLangList[i];
	            		html += '<div class=\"service_detail\">';
						html += '<h3>'+ (i+1) +'、'+ serverLang.title + '<span class="spanPrice">￥'+serverLang.viprmb+'.00</span></h3>';
						html += '<p>'+serverLang.desc+'</p>';
						if(serverLang.url != null && serverLang.url != ""){
							html += '<a href=\"./'+serverLang.url+'?type='+ serverLang.type+ '&price='+serverLang.viprmb + '&serviceId='+serverLang.servid + '&title='+serverLang.title + '\" class="button">详细介绍</a>';
							html += '<a href=\"./serviceOrder.html?type='+ serverLang.type + '&price='+serverLang.viprmb + '&serviceId='+serverLang.servid + '&title='+serverLang.title + '\" class=\"button\">我要购买</a></br>';
						}else{
