...
function init(){
	...
	setTimeout(function(){
		if(token == null || token == "" || token == undefined || token == "null"){
			...
			loadUserService(postdata)//type=suit
		}
	}, 500)
//	loadUserService(postdata);//type=suit
            ...
            if(flag=="0"){
            	...
            	if(serverLangList.length > 0){
	            	for(var i=0; i < serverLangList.length ; i++){
						...
						html += '<a href=\"./'+serverLang.url+'?type='+ serverLang.type+ '&title='+serverLang.title+ '&serviceId='+serverLang.servid + '\" class="button">详细介绍</a>';
            	...
            	if(serverLangList.length > 0){
	            	for(var i=0; i < serverLangList.length ; i++){
						...
						if(serverLang.url != null && serverLang.url != ""){
							html += '<a href=\"./'+serverLang.url+'?type='+ serverLang.type+ '&price='+serverLang.viprmb + '&title='+serverLang.title + '&serviceId='+serverLang.servid + '\" class="button">详细介绍</a>';
							html += '<a href=\"./serviceOrder.html?type='+ serverLang.type + '&price='+serverLang.viprmb + '&title='+serverLang.title + '&serviceId='+serverLang.servid + '\" class=\"button\">我要购买</a>';
//							html += '<a href=\"./'+serverLang.url+'\" target=\"_blank\" class="button">详细介绍</a>';
//							html += '<a href=\"./serviceOrder.html?type='+ serverLang.type + '\" target=\"_blank\" class=\"button\">我要购买</a>';
						}else{
